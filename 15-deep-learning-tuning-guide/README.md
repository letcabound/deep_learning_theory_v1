...
- 如果可以的话，请尝试**找到一篇尽可能接近手头问题的相关论文，并将复现该论文中的模型作为起点**。
...
-   增加Batch Size通常会减少训练时间。这非常有益，因为它：
    -   能使固定时间间隔内**超参数调整更彻底**，最终训练出更好的模型。
    -   **减少开发周期的延迟**，能更多地测试新想法。
-   资源消耗和Batch Size之间并没有明确的关系，增加Batch Size让资源消耗增加、减少或保持不变都有可能。
