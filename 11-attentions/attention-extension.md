...
## 6.1 具体实现原理
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;我们的主要目标是通过有效地在多个主机之间分配长序列，消除个别设备施加的内存限制，而无需增加额外开销。为实现这一目标，我们提出对块并行变压器（BPT）框架[23]进行增强。在将输入序列分布到不同主机时，每个主机负责运行与其指定块相对应的块并行注意力外部循环的一个元素，以及特定于该块的前馈网络。这些操作不需要与其他主机进行通信。然而，在涉及需要从其他主机获取块的键-值块交互的内部循环中出现了挑战。由于每个主机只拥有一个键-值块，从其他主机获取块的朴素方法会导致两个重要问题。首先，它会引入计算延迟，因为系统等待接收必要的键-值块。其次，键-值块的累积会导致内存使用增加，这将违背减少内存成本的初衷。<br>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;基于环形块并行注意力。为了解决上述挑战，我们利用内部循环键-值块操作的置换不变性属性。这一属性源自这样一个事实：查询块与一组键-值块之间的自注意力可以按任何顺序计算，只要每个块的统计数据正确组合以进行重新缩放。我们通过将所有主机概念化为形成一个环结构来利用这一属性：主机1、主机2，直至主机N。在计算块并行注意力和前馈时，每个主机通过同时将用于注意力计算的键-值块有效地发送到下一个主机，同时从前一个主机接收键-值块，有效地重叠块的传输与块级计算。具体而言，对于任何主机i，在计算其查询块与一个键-值块之间的注意力时，它同时将键-值块发送给下一个主机(i + 1)，同时从前一个主机(i - 1)接收键-值块。如果计算时间超过了传输键-值块所需的时间，这将不会导致额外的通信成本。这种重叠机制适用于我们方法的正向和反向传递，因为可以使用相同的操作和技术。先前的工作还提出利用环拓扑来计算自注意力，旨在降低通信成本。我们的工作不同之处在于利用块并行变压器大幅降低内存成本。正如我们在下一节中所展示的，这使得在训练和推理过程中上下文大小的扩展零开销，并允许任意大的上下文大小。<br>

![ring-attention-figure2](images/ring-attention-figure2.png)

*图a:我们使用与原始Transformer相同的模型架构，但重新组织计算。在图中，我们通过显示在主机环中，每个主机持有一个查询块，并且键-值块通过主机环进行遍历，以块对块的方式进行注意力和前馈计算来解释这一点。在计算注意力时，每个主机将键-值块发送到下一个主机，同时从前一个主机接收键-值块。通信与块级注意力和前馈计算重叠。*
*图b:我们逐块计算原始Transformer。每个主机负责查询外部循环的一个迭代，而键-值块在主机之间轮转。如图所示，一个设备从左侧开始处理第一个查询块；然后我们沿水平位置排列的键-值块序列进行迭代。查询块与键-值块结合用于计算自注意力（黄色框），其输出传递给前馈网络（青色框）。*
